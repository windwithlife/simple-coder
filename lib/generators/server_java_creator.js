...
var config = {
    ...
    targetServerRootPath:function(){return path.join(this.targetRootPath,"/autocoder/src/server/")},
    targetServerBasePath:function(){return path.join(this.targetServerRootPath(),this.language,xtools.javaPackageToPath(this.basePackage))},
    //targetServerJavaBZPath:function(){return path.join(this.targetServerJavaPath(),)},
...
function createServerBZModules(moduleName,moduleDefine){
    ...
    var targetFile   = config.targetServerBasePath() + "entity/" + mName + ".java";

    templateFile = config.workServerTemplatePath() + "Dao.java" ;
    //var clsName = nName + "Repository";
    targetFile   = config.targetServerBasePath() + "dao/" + mName+"Repository.java";
    codeTools.generateCode(templateFile,params,targetFile);

    templateFile = config.workServerTemplatePath() + "Service.java" ;
    targetFile   = config.targetServerBasePath() + "service/" + mName + "Service.java";
    codeTools.generateCode(templateFile,params,targetFile);


    templateFile = config.workServerTemplatePath() + "Controller.java" ;
    targetFile   = config.targetServerBasePath() + "controller/" + mName + "Controller.java";
    codeTools.generateCode(templateFile,params,targetFile);
};
...
function generateFrameworkDirectories(){
    ...
    xtools.mkdirX(config.targetServerBasePath());
    xtools.mkdirX(config.targetServerBasePath()+ 'entity');
    xtools.mkdirX(config.targetServerBasePath()+ 'controller');
    xtools.mkdirX(config.targetServerBasePath()+ 'service');
    xtools.mkdirX(config.targetServerBasePath()+ 'dao');
...
function copyProjectFrameworkFiles(){
    var frameworkServerPath = path.join(config.workCopyFilesTemplatePath(), "server/java");
...
exports.coderDefine = {name:"server-java",desc:"create a web framework and related project code"};
